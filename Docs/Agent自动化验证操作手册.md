...
4. 将实现文件和开发记录一起显式暂存。
5. 执行 `git diff --cached --check` 与 `git diff --cached --name-only`，确认暂存区包含记录且没有无关文件。
6. 最后执行中文 `git commit`。

开发记录使用“计划提交说明”关联对应提交，因为记录完成时提交哈希尚不存在。不得为了补哈希而 amend 已完成提交。若一次任务拆成生产修复、测试和文档三个提交，则必须分别创建三份记录，不能用一份任务总结覆盖多个提交。
