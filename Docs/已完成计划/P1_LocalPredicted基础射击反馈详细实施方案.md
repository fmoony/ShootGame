...
> **归档说明（2026-09-17）**：P1 已完成并归档；本文保留为实施依据与历史证据，不再更新。
> 完成提交与验证证据见同目录执行计划文档顶部的归档说明：
> [P1 Local Predicted 基础射击反馈执行计划](P1_LocalPredicted基础射击反馈执行计划.md)
...
自 2026-09-16 起，最终验收、证据有效性与交付报告还必须遵循
[网络射击 AI 自主验证契约](../架构/网络射击AI自主验证契约.md)。本文保留的 P1 子阶段、Phase、Case、
计数器和建议测试名均属于 Agent 内部实现细节；它们不能单独替代五条顶层不变量的通过证据。

```text
上游计划（范围与验收）
→ 本文（具体实施方案）+ 网络射击 AI 自主验证契约（最终验收与报告）
→ 每个子阶段一份开发记录 + 一个独立提交
```
...
本节列出的 RPC、门闩和日志字段是形成证据的一种实现建议，不是对外验收接口。Agent 应先依据
[网络射击 AI 自主验证契约](../架构/网络射击AI自主验证契约.md) 判断现有证据是否有效；能由普通
Automation 证明的服务器本地规则不得继续堆入 Coordinator，已有网络场景能够直接取证时不得仅为
增加覆盖数量创建新 Phase。Coordinator 总门通过不能覆盖单项证据无效。
