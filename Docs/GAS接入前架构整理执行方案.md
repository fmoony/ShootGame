...
本轮只整理现有架构，不接入 Gameplay Ability System。执行完成后，才允许从 [GAS、动画分层与客户端预测扩展计划](GAS动画分层与预测扩展计划.md) 的“阶段 1：只接入 GAS 模块”继续。
...
- 更新 [GAS、动画分层与客户端预测扩展计划](GAS动画分层与预测扩展计划.md) 中的实际目录入口。
