...
- [动画分层与射击表现扩展规划](Docs/执行计划/动画分层与射击表现扩展规划.md)：当前候选表现路线；规划第三人称动画分层、瞄准同步、Reload / Equip 动画、角度散布与射击手感。
- [第三人称动画分层实施计划](Docs/执行计划/第三人称动画分层实施计划.md)：当前执行入口；分步调整 Rifle / Pistol 上下半身混合，优先定位并修正跳跃姿势对瞄准和枪口方向的干扰。
