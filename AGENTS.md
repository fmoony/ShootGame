...
- [Inventory 第二阶段：基础闭环执行计划](Docs/已完成计划/Inventory第二阶段_基础闭环执行计划.md)：已完成；记录 WeaponInstance、OwnerOnly FastArray、当前武器和弹药权威闭环。
- [GA_Fire ServerOnly 执行计划](Docs/已完成计划/GA_Fire_ServerOnly执行计划.md)：已完成（2026-08-20）；记录 Ability 输入、玩家/NPC 开火、取消边界和旧 RPC 清理。
