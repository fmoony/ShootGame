...
namespace ShooterWeaponRowCompleteness
{
	...
	bool TestRow(FAutomationTestBase& Test, const TCHAR* RowName)
	{
		...
		const FShooterWeaponConfigRow* Row = ShooterWeaponTable::FindWeaponRow(
			...
			FName(RowName));
		if (!Test.TestNotNull(
			FString::Printf(TEXT("Row %s resolves"), RowName),
			Row))
		{
			return false;
		}
		...
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s is valid for grant"), RowName),
			ShooterWeaponTable::IsRowValidForGrant(Row));

		// 数值区间：非法值必须在数据层就被拒绝，而不是留到运行时。
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s magazine size is positive"), RowName),
			Row->MagazineSize > 0 && Row->MagazineSize <= 100);
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s reserve declaration is >= -1"), RowName),
			Row->InitialReserveAmmo >= -1);
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s refire rate is within range"), RowName),
			Row->RefireRate > 0.0f && Row->RefireRate <= 5.0f);
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s aim variance is within range"), RowName),
			Row->AimVariance >= 0.0f && Row->AimVariance <= 90.0f);
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s muzzle offset is non-negative"), RowName),
			Row->MuzzleOffset >= 0.0f);
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s reload duration is positive"), RowName),
			Row->ReloadDuration > 0.0f);
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s equip duration is non-negative"), RowName),
		...
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s declares a muzzle socket"), RowName),
			!Row->MuzzleSocketName.IsNone());

		// 资源引用：类引用必须是具体子类，网格/动画/表现资源必须真实存在。
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s WeaponActorClass derives from AShooterWeapon"), RowName),
		...
		bValid &= Test.TestTrue(
			...
			Row->FireBehaviorClass &&
			Row->FireBehaviorClass->IsChildOf<UShooterWeaponFireBehavior>());
		bValid &= Test.TestTrue(
		...
		bValid &= Test.TestTrue(
			...
			Row->FirstPersonAnimInstanceClass &&
			Row->FirstPersonAnimInstanceClass->IsChildOf<UAnimInstance>());
		bValid &= Test.TestTrue(
			FString::Printf(TEXT("Row %s ThirdPersonAnimInstanceClass derives from UAnimInstance"), RowName),
			Row->ThirdPersonAnimInstanceClass &&
			Row->ThirdPersonAnimInstanceClass->IsChildOf<UAnimInstance>());

		bValid &= Test.TestNotNull(
			FString::Printf(TEXT("Row %s first-person mesh resolves"), RowName),
			Row->FirstPersonMesh.LoadSynchronous());
		bValid &= Test.TestNotNull(
			FString::Printf(TEXT("Row %s third-person mesh resolves"), RowName),
			Row->ThirdPersonMesh.LoadSynchronous());
		bValid &= Test.TestNotNull(
			FString::Printf(TEXT("Row %s pickup mesh resolves"), RowName),
			Row->PickupMesh.LoadSynchronous());
		bValid &= Test.TestNotNull(
			FString::Printf(TEXT("Row %s firing montage resolves"), RowName),
			Row->FiringMontage.Get());
		bValid &= Test.TestNotNull(
			FString::Printf(TEXT("Row %s fire sound resolves"), RowName),
			Row->FireSound.Get());
		bValid &= Test.TestNotNull(
			FString::Printf(TEXT("Row %s reload magazine-out sound resolves"), RowName),
			Row->ReloadMagazineOutSound.Get());
		bValid &= Test.TestNotNull(
			FString::Printf(TEXT("Row %s reload magazine-in sound resolves"), RowName),
			Row->ReloadMagazineInSound.Get());
		bValid &= Test.TestNotNull(
			FString::Printf(TEXT("Row %s reload cocking sound resolves"), RowName),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FShooterWeaponRowCompletenessTest,
	"ShootGame.Weapon.ProductionRowsResolvable",
...
bool FShooterWeaponRowCompletenessTest::RunTest(const FString& Parameters)
{
	...
	{
		TestEqual(
			TEXT("Weapon table contains exactly the declared production rows"),
